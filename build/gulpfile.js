...
//COPIES
function copyFonts() {
  return src([SRC_PATHS.font]).pipe(dest(PUBLIC_PATHS.font));
}

function copyFiles() {
  return src([SRC_PATHS.files]).pipe(dest(PUBLIC_PATHS.files));
}

function copyRootFiles() {
  return src([SRC_PATHS.rootFiles]).pipe(dest(PUBLIC_PATHS.html));
}
...
const build = series(
  clearPublic,
  parallel(htmlPublic, js, css, svg, img, copyFonts, copyFiles, copyRootFiles),
);
